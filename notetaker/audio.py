...
Captures lecture audio from PulseAudio/PipeWire via ffmpeg, writing both:
...
Two source kinds matter:
  ...
  system  - a ".monitor" source, i.e. what the speakers are playing
            (online lecture: Zoom / Teams / YouTube)
...
class AudioSource:
    ...
    name: str  # raw device name passed to ffmpeg
...
def list_sources() -> list[AudioSource]:
    """Enumerate capture sources, labelling monitors as system audio.
...
def resolve_source(selector: str | None) -> AudioSource:
    ...
    if not sources:
        raise AudioError("no audio capture sources found (is PipeWire/PulseAudio running?)")
    ...
    raise AudioError(
        ...
        f"Use 'mic', 'system', or one of: {', '.join(s.name for s in sources)}"
    )
...
class Recorder:
    ...
    def start(self) -> None:
        ...
        if self._proc.poll() is not None:
            ...
            raise AudioError(f"ffmpeg failed to start on {self.source.name!r}: {err.strip()}")
