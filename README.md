...
notetaker record --source mic          # in-person lecture, Ctrl-C to stop
notetaker record --source system       # online lecture
notetaker list                         # past recordings
notetaker show <id>                    # read the notes
```

When you stop the recording it transcribes the last chunk, then prints and
saves the key ideas.

### Live notes while recording

```bash
notetaker record --source mic --live-notes
```

Key ideas appear every ~3 minutes as the lecture goes. It costs extra CPU, so
it is off by default. A full summary still runs at the end either way.

### Other things you can do

```bash
notetaker record --title "Thermodynamics week 4"
notetaker record --lang th              # skip autodetection
notetaker summarize <id> --rerun        # redo the notes, no re-recording
notetaker summarize <id> --hq           # re-transcribe with a better model
notetaker export <id> -o notes.md       # notes as markdown
notetaker export <id> --txt             # raw transcript
```

`<id>` can be part of the title, so `notetaker show thermo` works.
...
**Transcription** — real-time factor **0.62** with the `small` model, so it
comfortably keeps up with a live lecture with ~38% headroom.
...
- **Thai accuracy** on the `small` model is noticeably weaker than English. Run
  `notetaker summarize <id> --hq` afterwards to re-transcribe with
  `large-v3-turbo`, which is much slower but considerably better.
- **Thai summaries occasionally invent a term.** Check anything that matters.
